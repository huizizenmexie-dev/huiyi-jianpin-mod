...
- [x] 保存清理后的新版本并向用户汇报结果
- [x] 将当前静态网站升级为支持后端、数据库与存储能力的全栈项目
- [x] 为项目接入文件存储并设计基础上传与访问流程
- [x] 验证文件上传、读取与管理链路是否可用
- [x] 保存全栈升级后的检查点并向用户汇报结果
- [x] 将用户提供的第三方站点脚本接入网站模板的 head 或 body
- [x] 验证脚本已写入页面模板且项目可以正常构建

- [x] 回退未完成的全栈与文件存储相关改动，仅保留当前前端站点
- [x] 检查前端页面、测试与构建状态，确认当前站点可继续使用
- [x] 保存仅保留前端站点的新检查点并向用户汇报结果
